...
class TestPHL1(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.bin_df = hme.utils.make_bin_gdf_from_rupture_gdf(
    ...
    def test_make_bin_gdf_from_rupture_gdf(self):
        bin_df = hme.utils.make_bin_gdf_from_rupture_gdf(
            self.rup_gdf, h3_res=3, parallel=False
        )
        self.assertEqual(
    ...
    def test_add_ruptures_to_bins(self):
        ...
        num_rups_first_bin = len(
            self.bin_df.SpacemagBin.loc["836860fffffffff"].mag_bins[6.2].ruptures
        )

        self.assertEqual(num_rups_first_bin, 561)

    def test_make_earthquake_gdf(self):
        self.eq_df = hme.utils.make_earthquake_gdf_from_csv(
    ...
    def test_make_SpacemagBins_from_bin_gdf(self):
        self.assertIsInstance(self.bin_df.iloc[0].SpacemagBin, hme.utils.SpacemagBin)
