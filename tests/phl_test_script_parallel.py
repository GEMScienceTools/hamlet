...
hztest.utils.make_SpacemagBins_from_bin_df(bin_df, min_mag=min_bin_mag,
                                           bin_width=bin_width)
...
t0 = time.time()
rl = hztest.utils.rupture_list_from_lt_branch_parallel(phlt['b1'], 
...
def update_mfd_counts(mfd_counts, sb, interval_length):
    ...
    for bc, n_eqs in sb.get_rupture_sample_mfd(interval_length,
                                               normalize=False,
                                               cumulative=False).items()]

def get_stoch_mfd(sb, interval_length=40, iters=mfd_iters, bcs=bcs):
    mfd_counts = {bc: [] for bc in bcs}

    _ = [update_mfd_counts(mfd_counts, sb, interval_length=interval_length)
    ...
    for bc, eq_counts in mfd_counts.items():
            n_eqs, n_occurrences = np.unique(eq_counts, return_counts=True)
            mfd_freq_counts[bc] = {n_eq: n_occurrences[i] / sum(n_occurrences)
                                   for i, n_eq in enumerate(n_eqs)}
    return mfd_freq_counts
...
def stoch_mfd_apply(series): return series.apply(get_stoch_mfd)
...
def calc_mfd_log_likelihood(obs_eqs, mfd_dict):
    ...
    return np.exp(np.sum(np.log([calc_bin_likelihood(bc, len(eqs), mfd_dict)
                         for bc, eqs in obs_eqs.items()])) / n_bins)
...
source_bin_df['log_like'] = source_bin_df.apply(calc_row_log_likelihood, axis=1)
...
source_bin_df[['idx', 'log_like', 'geometry']].to_file('/home/itchy/Desktop/phl_bin_likes.geojson', driver='GeoJSON')