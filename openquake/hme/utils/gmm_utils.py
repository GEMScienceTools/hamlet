...
def evaluate_gmc(test_config, input_data):
    ...
    # Get the min number of records to use
    min_recordings = test_config.get("min_recordings_per_event", 3)
    ...
    for trt in set(eq_trt_map.values()):
        ...
        # Keep only earthquakes with at least min_recordings recordings
        rec_counts = trt_records.groupby("event_id").size()
        val_ids = rec_counts[rec_counts >= min_recordings].index
        trt_records = trt_records[trt_records["event_id"].isin(val_ids)]
        eq_subset = eq_subset[eq_subset["event_id"].isin(val_ids)]

        if len(eq_subset) == 0:
            logging.info(
                f"No events with >= {min_recordings} recordings for TRT {trt}, skipping"
            )
            continue
