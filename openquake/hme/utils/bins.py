    ...
    def to_dict(self):
        ...
        if self.observed_earthquakes == []:
            ...
            observed_earthquakes = {
                'observed_earthquakes':
                [e.__dict__ for e in self.observed_earthquakes]
            }

        return {**params, **ruptures, **observed_earthquakes}
...
class SpacemagBin():
    ...
    def get_bin_edges(self):
        ...
        edges.append(self.mag_bin_centers[-1] + self.bin_width / 2.)
    ...
    def to_dict(self):
        ...
        mag_bins = {
            'mag_bins':
            {bc: mb.to_dict()
             for (bc, mb) in self.mag_bins.items()}
        }
    ...
    def sample_ruptures(self, interval_length, t0=0., clean=True):
        for bc, mag_bin in self.mag_bins.items():
            ...
            if clean is True:
                self.stochastic_earthquakes[
                    bc] = mag_bin.stochastic_earthquakes
            else:
    ...
    def get_rupture_mfd(self, cumulative=False):
        ...
        noncum_mfd = {
            bc:
            self.mag_bins[bc].calculate_total_rupture_rate(return_rate=True)
            for bc in self.mag_bin_centers
        }
    ...
    def get_rupture_sample_mfd(self,
                               interval_length,
                               t0=0.,
                               normalize=True,
                               cumulative=False):
