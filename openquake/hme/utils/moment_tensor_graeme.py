...
def symmat6(*vals):
    ...
    return num.matrix(
        [[vals[0], vals[3], vals[4]], [vals[3], vals[1], vals[5]],
...
def euler_to_matrix(alpha, beta, gamma):
    ...
    :param alpha: 
        The angle between the z-axis and the zs-axis.

    :param beta: 
        The angle between the x-axis and the line of nodes.

    :param gamma: 
        The angle between the line of nodes and the xs-axis.
...
class MomentTensor:
    ...
    def __str__(self):
        ...
        s = s % (self.scalar_moment(), self.moment_magnitude(), m[0, 0],
...
def other_plane(strike, dip, rake):
    ...
    w = [
        sum([abs(x - y) for x, y in zip(both_sdr[i], (strike, dip, rake))])
        for i in (0, 1)
    ]
    if w[0] < w[1]:
        return both_sdr[1]
