...
def test_calc_mag_bin_empirical_likelihood_no_events():
    ...
    assert (
        calc_mag_bin_empirical_likelihood(2, rate_dict, not_modeled_val=1e-5)
        == 1e-5
    )
...
def test_calc_mag_bin_poisson_likelihood_events():
    ...
    my_probs = np.array(
        [calc_mag_bin_poisson_likelihood(n, rate) for n in n_events]
    )
...
def test_calc_mag_bin_poisson_likelihood_no_events():
    prob_no_events = calc_mag_bin_poisson_likelihood(
        1, 0.0, not_modeled_val=1e-5
    )
...
def test_calc_mag_bin_likelihood_w_poisson():
    prob = calc_mag_bin_likelihood(
        1, 0.02, time_interval=1.0, likelihood_method="poisson"
    )
